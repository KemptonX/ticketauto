...
// ─── ScaledPreview — shrinks fixed-size banner to fit any container width ─────

function ScaledPreview({ children, size = 460 }: { children: React.ReactNode; size?: number }) {
  ...
  useEffect(() => {
    ...
    const obs = new ResizeObserver(() => {
      ...
      if (w > 0) setScale(Math.min(1, w / size));
    });
    obs.observe(el);
  ...
  return (
    <div ref={ref} style={{ width: "100%", height: Math.round(size * scale), position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", top: 0, left: 0, width: size, height: size, transform: `scale(${scale})`, transformOrigin: "top left" }}>
        {children}
      </div>
    </div>
  );
...
// ─── Sale banner (React preview, 460 × 460) ───────────────────────────────────
...
export function SaleBanner({ sale, order, animated, hideDetails = false }: BannerProps) {
  ...
  return (
    <div style={{
      ...
      {/* Watermark X — bottom-right, clear of stat boxes */}
      <div style={{ position: "absolute", bottom: 52, right: 16, opacity: 0.03, pointerEvents: "none", lineHeight: 0 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 22px 0", flexShrink: 0 }}>
        <img src="/logo.png" style={{ height: 26, width: "auto" }} alt="TixTracker" />
      ...
      <div style={{ padding: "11px 22px 0", flexShrink: 0 }}>
        ...
        <div style={{ marginTop: 3, display: "flex", alignItems: "center", gap: 5, flexWrap: "nowrap", overflow: "hidden" }}>
          {s.venue && (
            <span style={{ fontSize: 11, color: "rgba(255,255,255,0.36)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 210, flexShrink: 1 }}>
              {s.venue}
            </span>
          )}
      ...
      {/* ── Hero profit — centred in remaining space ── */}
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 22px" }}>
        ...
        <div style={{ fontSize: 52, fontWeight: 800, letterSpacing: "-0.04em", color: profitColor, textShadow: profitGlow, lineHeight: 1, fontVariantNumeric: "tabular-nums" }}>
        ...
        {roi !== null && (
          <div style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 6, background: isPos ? "rgba(74,222,128,0.09)" : "rgba(248,113,113,0.09)", border: `1px solid ${isPos ? "rgba(74,222,128,0.20)" : "rgba(248,113,113,0.20)"}`, borderRadius: 999, padding: "5px 14px" }}>
            <span style={{ fontSize: 16, fontWeight: 800, letterSpacing: "-0.02em", color: profitColor, fontVariantNumeric: "tabular-nums" }}>
              {isPos ? "+" : "−"}{Math.abs(dRoi).toFixed(1)}%
            </span>
            <span style={{ fontSize: 9.5, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: profitColor, opacity: 0.65 }}>
              ROI
            </span>
          </div>
        )}
      ...
      {/* ── Footer ── */}
      <div style={{ padding: "9px 22px 16px", borderTop: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
...
// ─── Multi-sale banner (React preview, 460 × 460) ─────────────────────────────
        ...
        {eventNames.length === 1 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {eventNames.slice(0, 3).map((name, i) => (
              <div key={i} style={{ fontSize: eventNames.length === 2 ? 13 : 11.5, fontWeight: 700, letterSpacing: "-0.02em", color: "rgba(255,255,255,0.92)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                {name}
              </div>
            ))}
            {eventNames.length > 3 && (
              <div style={{ fontSize: 10.5, color: "rgba(255,255,255,0.28)" }}>+{eventNames.length - 3} more</div>
            )}
          </div>
        )}
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 22px" }}>
        ...
        {roi !== null && (
          <div style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 6, background: isPos ? "rgba(74,222,128,0.09)" : "rgba(248,113,113,0.09)", border: `1px solid ${isPos ? "rgba(74,222,128,0.20)" : "rgba(248,113,113,0.20)"}`, borderRadius: 999, padding: "5px 14px" }}>
            ...
            <span style={{ fontSize: 9.5, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: profitColor, opacity: 0.65 }}>ROI</span>
          </div>
        )}
      </div>
...
function _txMark(ctx: CanvasRenderingContext2D, x: number, y: number, cssSize: number, sc: number) {
  ...
  ctx.beginPath(); ctx.moveTo(x + 4.5 * r, y + 4.5 * r); ctx.lineTo(x + 19.5 * r, y + 19.5 * r); ctx.stroke();
  ctx.strokeStyle = "#FF4FA3";
  ctx.beginPath(); ctx.moveTo(x + 19.5 * r, y + 4.5 * r); ctx.lineTo(x + 4.5 * r, y + 19.5 * r); ctx.stroke();
  ctx.restore();
}
...
function _bannerBg(ctx: CanvasRenderingContext2D, W: number, H: number, isPos: boolean, sc: number) {
  ...
  ctx.fillRect(0, 0, W * 0.5, H * 0.4);
  ctx.fillStyle = "rgba(255,79,163,0.06)";
  ...
  const ws = 110 * sc, wr = ws / 24;
  const wx = W - 16 * sc - ws, wy = H - 52 * sc - ws;
  ctx.lineCap = "round"; ctx.lineWidth = 3.5 * wr;
  ctx.strokeStyle = "white";
  ctx.beginPath(); ctx.moveTo(wx + 4.5 * wr, wy + 4.5 * wr); ctx.lineTo(wx + 19.5 * wr, wy + 19.5 * wr); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(wx + 19.5 * wr, wy + 4.5 * wr); ctx.lineTo(wx + 4.5 * wr, wy + 19.5 * wr); ctx.stroke();
  ctx.restore();
}
...
function _statBox(
  ...
  ctx.fillStyle = pink ? "rgba(255,79,163,0.05)" : "rgba(255,255,255,0.04)";
  ctx.fill();
  ...
  ctx.fillText(label, ix, iy); iy += (8.5 + 5) * sc;
  ctx.restore();
  ctx.save();
  ctx.font = `700 ${18 * sc}px ${_F}`; ctx.fillStyle = "rgba(255,255,255,0.88)";
  ctx.textAlign = "left"; ctx.textBaseline = "top";
  ctx.fillText(value, ix, iy); iy += (18 + 3) * sc;
  ctx.restore();
  ctx.save();
...
function _heroSection(
  ...
  const bigSz = 52 * sc, labelSz = 9 * sc;
  const roiPillH = roi !== null ? (5 * 2 + 16) * sc : 0;
  const gap1 = 8 * sc, gap2 = 10 * sc;
  const contentH = labelSz + gap1 + bigSz + (roi !== null ? gap2 + roiPillH : 0);
  ...
  ctx.fillStyle = profitColor; ctx.fillText(profitStr, W / 2, numTop);
  ctx.restore();
  ...
  if (roi !== null) {
    ...
    ctx.save();
    ctx.font = `800 ${16 * sc}px ${_F}`; const roiNumW = ctx.measureText(roiStr).width;
    ctx.font = `600 ${9.5 * sc}px ${_F}`; const roiLabelW = ctx.measureText(roiLabel).width;
    ctx.restore();
    const pillW = pw + roiNumW + gap + roiLabelW + pw;
    const pillH = 16 * sc + ph * 2;
    ...
    ctx.fillStyle = profitColor; ctx.fillText(roiStr, pillX + pw, pillCY);
    ctx.restore();
    ctx.save();
    ctx.font = `600 ${9.5 * sc}px ${_F}`; ctx.textAlign = "left"; ctx.textBaseline = "middle";
    ctx.fillStyle = profitColor; ctx.globalAlpha = 0.65;
...
function _bannerFooter(
  ...
  rightText: string, logo: HTMLImageElement | null = null,
) {
  ctx.save();
  ...
  if (logo) {
    ...
    _txMark(ctx, spad, fY, 16, sc);
    textX = spad + 16 * sc + 6 * sc;
  }
  ctx.save();
...
// ─── Canvas: draw single-sale banner ─────────────────────────────────────────
...
function _drawSaleBanner(
  ...
  if (logo) {
    ...
    _txMark(ctx, spad, topY, 20, sc);
    ctx.save();
    ctx.font = `700 ${14 * sc}px ${_F}`; ctx.textBaseline = "middle"; ctx.textAlign = "left";
    ctx.fillStyle = "rgba(255,255,255,0.9)";
    const txX = spad + 20 * sc + 8 * sc;
    ctx.fillText("Tix", txX, logoCY);
    ctx.fillStyle = "#b87bff"; ctx.fillText("Tracker", txX + ctx.measureText("Tix").width, logoCY);
    ctx.restore();
  }
  ...
  ctx.save();
  ctx.font = `700 ${13.5 * sc}px ${_F}`; ctx.textBaseline = "top"; ctx.textAlign = "left";
  ...
  if (sale.venue) {
    ...
    const vStr = _clip(ctx, sale.venue, 200 * sc);
    ctx.fillText(vStr, curX, venueY); curX += ctx.measureText(vStr).width;
    if (sale.event_date) { ctx.fillStyle = "rgba(255,255,255,0.18)"; ctx.fillText(" · ", curX, venueY); curX += ctx.measureText(" · ").width; }
  }
  if (sale.event_date) { ctx.fillStyle = "rgba(255,255,255,0.28)"; ctx.fillText(formatEventDate(sale.event_date), curX, venueY); }
  ctx.restore();

  const divY = venueY + 11 * sc * 1.3 + 10 * sc;
  ctx.fillStyle = "rgba(255,255,255,0.05)"; ctx.fillRect(spad, divY, W - spad * 2, sc);

  // Footer + stat box positions (work up from bottom)
  const footerH = 41 * sc;
  ...
  _statBox(ctx, bOL, boxTop, bW, boxH, "BOUGHT FOR",
    ...
    sc, false);
  _statBox(ctx, bOL + bW + boxGap, boxTop, bW, boxH, "SOLD FOR",
    revenue > 0 ? formatCurrency(revenue) : "—",
    `via ${formatSource(sale.source)}`, sc, true);

  const rightFooter = sale.section ? sale.section + (sale.row ? ` · Row ${sale.row}` : "") : "";
  _bannerFooter(ctx, W, footerTop, spad, sc, rightFooter, logo);

  _heroSection(ctx, W, divY + sc, boxTop, (isPos ? "" : "−") + formatCurrency(Math.abs(profit)), roi, isPos, profitColor, sc);
}

// ─── Canvas: draw multi-sale banner ──────────────────────────────────────────
...
function _drawMultiBanner(
  ...
  const spad = 22 * sc, topY = 20 * sc;
  const logoH = 26 * sc, logoCY = topY + logoH / 2;
  ...
  const eventY = topY + logoH + 11 * sc;
  let eventBottom = eventY;

  if (eventNames.length === 1) {
    ctx.save();
    ctx.font = `700 ${13.5 * sc}px ${_F}`; ctx.textBaseline = "top"; ctx.textAlign = "left";
    ctx.fillStyle = "rgba(255,255,255,0.95)";
    ctx.fillText(_clip(ctx, eventNames[0], W - spad * 2), spad, eventY);
    ctx.restore();
    eventBottom = eventY + 13.5 * sc * 1.25;
  } else {
    ...
    for (const name of eventNames.slice(0, 3)) {
      ctx.save();
      ctx.font = `700 ${nfs * sc}px ${_F}`; ctx.textBaseline = "top"; ctx.textAlign = "left";
  ...
  const divY = subY + 11 * sc * 1.3 + 10 * sc;
  ctx.fillStyle = "rgba(255,255,255,0.05)"; ctx.fillRect(spad, divY, W - spad * 2, sc);
  ...
  _statBox(ctx, bOL + bW + boxGap, boxTop, bW, boxH, "TOTAL REVENUE",
  ...
  _heroSection(ctx, W, divY + sc, boxTop, (isPos ? "" : "−") + formatCurrency(Math.abs(totalProfit)), roi, isPos, profitColor, sc);
}
...
async function renderBannerToBlob(
  ...
  if (typeof OffscreenCanvas !== "undefined") {
    try {
      ...
      if (ctx) { drawFn(ctx, W, H, logo); return await oc.convertToBlob({ type: "image/jpeg", quality: 0.92 }); }
    } catch { /* fall through */ }
  }
  ...
  try {
    ...
    if (!dctx) throw new Error("no ctx");
    drawFn(dctx, W, H, logo);
    const id = dctx.getImageData(0, 0, W, H);
    const bm = await createImageBitmap(id);
    const ec = document.createElement("canvas"); ec.width = W; ec.height = H;
    const ectx = ec.getContext("2d", { willReadFrequently: true });
    if (!ectx) throw new Error("no enc ctx");
    ectx.drawImage(bm, 0, 0); bm.close();
    return await new Promise<Blob>((res, rej) => ec.toBlob(b => b ? res(b) : rej(new Error("toBlob null")), "image/jpeg", 0.92));
  } catch { /* fall through */ }
  ...
  drawFn(ctx, W, H, logo);
  return new Promise<Blob>((res, rej) => canvas.toBlob(b => b ? res(b) : rej(new Error("toBlob null")), "image/jpeg", 0.92));
}
...
export function ShareMultiBannerModal({ stats, onClose }: MultiShareProps) {
  ...
  function handleDownload() {
    if (!bannerBlob) return;
    _downloadBlob(bannerBlob, "sales-summary-tixtracker.jpg");
  }
    ...
    >
      <div style={{ display: "flex", flexDirection: "column", gap: 14, width: "100%", maxWidth: 500, transform: visible ? "scale(1) translateY(0)" : "scale(0.94) translateY(10px)", transition: "transform 300ms cubic-bezier(0.16,1,0.3,1), opacity 300ms ease", opacity: visible ? 1 : 0, position: "relative", margin: "auto" }}>
        ...
        <div style={{ borderRadius: 16, overflow: "hidden", boxShadow: "0 0 0 1px rgba(255,255,255,0.08), 0 20px 50px rgba(0,0,0,0.6), 0 0 36px rgba(155,92,255,0.09)" }}>
          <ScaledPreview size={460}><MultiSaleBanner stats={stats} animated={visible} /></ScaledPreview>
        </div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button type="button" onClick={handleCopy} disabled={!bannerBlob || copying} style={{ ...btnBase, flex: 1, background: copied ? "rgba(74,222,128,0.14)" : !bannerBlob ? "rgba(255,255,255,0.04)" : "linear-gradient(135deg, rgba(155,92,255,0.22), rgba(255,79,163,0.22))", border: copied ? "1px solid rgba(74,222,128,0.35)" : "1px solid rgba(255,79,163,0.25)", color: copied ? "#4ade80" : "rgba(255,255,255,0.9)", cursor: (!bannerBlob || copying) ? "wait" : "pointer" }}>
            {!bannerBlob && !genError ? "Generating…" : copied ? "✓ Copied!" : copying ? "Copying…" : "Copy to Clipboard"}
          </button>
          <button type="button" onClick={handleDownload} disabled={!bannerBlob} style={{ ...btnBase, padding: "0 20px", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.10)", color: "rgba(255,255,255,0.55)", cursor: !bannerBlob ? "wait" : "pointer", whiteSpace: "nowrap" }}>
            Download
          </button>
        </div>
      </div>
...
export default function ShareBannerModal({ sale, order, onClose }: Props) {
  ...
  function handleCopy() {
    if (!bannerBlob) return;
    const blob = bannerBlob;
    const filename = `${sale.event_name ?? "sale"}-tixtracker.jpg`;
    setCopying(true);
    if (typeof ClipboardItem !== "undefined" && navigator.clipboard?.write) {
      void navigator.clipboard.write([new ClipboardItem({ "image/jpeg": blob })])
        .then(() => { setCopied(true); setTimeout(() => setCopied(false), 2500); })
        .catch(() => { _downloadBlob(blob, filename); setCopied(true); setTimeout(() => setCopied(false), 2500); })
        .finally(() => setCopying(false));
    } else {
  ...
  function handleDownload() {
    if (!bannerBlob) return;
    _downloadBlob(bannerBlob, `${sale.event_name ?? "sale"}-tixtracker.jpg`);
  }
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
            <button
              ...
              style={{ ...btnBase, height: 34, padding: "0 13px", background: hideDetails ? "rgba(155,92,255,0.18)" : "rgba(255,255,255,0.06)", border: hideDetails ? "1px solid rgba(155,92,255,0.40)" : "1px solid rgba(255,255,255,0.10)", borderRadius: 9, color: hideDetails ? "#b87bff" : "rgba(255,255,255,0.50)", fontSize: 12 }}
            >
