...
exports.runViagogoListing = runViagogoListing;
...
async function pollForOtp(accountId, timeoutMs = 300_000) {
...
async function runViagogoListing(browser, job, report) {
    ...
    try {
        ...
        if (job.account.sessionData?.cookies?.length) {
            await context.addCookies(job.account.sessionData.cookies);
        ...
        await report("listed", {
            ...
            encryptedSession: (0, crypto_js_1.encrypt)(JSON.stringify({ cookies: finalCookies })),
        });
...
// ── Step handlers ─────────────────────────────────────────────────────────────
async function handleQuantityStep(page, quantity) {
...
async function handleSplitRuleStep(page, splitRule) {
...
async function handleTicketTypeStep(page, storageProvider) {
    const bodyText = (await page.textContent("body").catch(() => "")) ?? "";
...
async function handleSeatDetailsStep(page, section, row, seatFrom, seatTo) {
    const bodyText = (await page.textContent("body").catch(() => "")) ?? "";
    ...
    await fillIfFound('input[name*="section" i], input[placeholder*="section" i], input[id*="section" i], input[aria-label*="section" i]', section, "section");
    await fillIfFound('input[name*="row" i], input[placeholder*="row" i], input[id*="row" i], input[aria-label*="row" i]', row, "row");
    if (seatFrom && seatTo) {
        // Try "from" and "to" seat inputs
        await fillIfFound('input[name*="seat_from" i], input[name*="seatfrom" i], input[placeholder*="from" i], input[aria-label*="first seat" i]', seatFrom, "seat from");
        await fillIfFound('input[name*="seat_to" i], input[name*="seatto" i], input[placeholder*="to" i], input[aria-label*="last seat" i]', seatTo, "seat to");
        // Viagogo sometimes uses a single "seats" input as a range (e.g., "5-7")
        const seatSingle = page.locator('input[name*="seat" i]:not([name*="from" i]):not([name*="to" i]), input[placeholder*="seat number" i]').first();
...
async function handlePriceStep(page, pricePerTicket, faceValuePerTicket) {
...
async function handleFeaturesStep(page, _features, _restrictions) {
...
async function handleTwoFactor(page, context, accountId, report) {
    const cookies = await context.cookies();
    await report("verification_required", {
        pendingVerification: true,
        encryptedSession: (0, crypto_js_1.encrypt)(JSON.stringify({ cookies })),
    });
    console.log(`[viagogo] 2FA required — waiting up to 5 min for OTP from user`);
    const otp = await pollForOtp(accountId);
    if (!otp)
        throw new Error("Timed out waiting for 2FA code — enter it in TixTracker and retry");
    console.log(`[viagogo] OTP received, entering code`);
    const otpInput = page.locator('input[type="text"], input[type="number"], input[autocomplete*="one-time"], input[name*="code" i], input[id*="code" i], input[placeholder*="code" i]').first();
...
async function dismissCookieBanner(page) {
    try {
        const btn = page.locator('button:has-text("Accept all"), button:has-text("Accept"), button:has-text("Allow all"), [id*="cookie"] button:has-text("OK")').first();
...
async function waitForAnyVisible(page, selectors) {
